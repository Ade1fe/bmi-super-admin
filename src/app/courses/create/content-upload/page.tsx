...
// ---------------------------------------------------------------------------
// QuestionRow — single question inside QuizSettingsModal
// ---------------------------------------------------------------------------
type DraftQuestion = {
  key: string; // local identity (not a real id until saved)
  question: string;
  type: QuizQuestionType;
  marks: number;
  options: string[];      // always 4 strings; ignored for theory
  correctOption: number;  // index; ignored for theory
  order: number;
};

function emptyQuestion(order: number): DraftQuestion {
  return {
    key: `q-${Date.now()}-${order}`,
    question: "",
...
function QuestionRow({
  ...
  return (
    <div className="rounded-[18px] border border-[#d4ddf0] bg-white p-5">
      <div className="flex items-center justify-between gap-4">
        <span className="text-[13px] font-extrabold uppercase tracking-[0.08em] text-[#0f8751]">
          Question {index + 1}
        </span>
        <button
          type="button"
          onClick={onDelete}
          className="flex h-8 w-8 items-center justify-center rounded-full text-[#7b899f] hover:bg-[#fff1f1] hover:text-[#c0392b]"
      ...
      <div className="mt-4 grid gap-4">
        {/* Question text */}
        <div>
          <span className="mb-2 block text-[13px] font-semibold text-[#51627f]">
            Question Text
          </span>
          <textarea
            rows={2}
            ...
            className="w-full resize-none rounded-[12px] border border-[#d7deee] bg-[#f8faff] px-4 py-3 text-[14px] leading-6 text-[#264267] outline-none placeholder:text-[#8d99b1]"
          />
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {/* Type */}
          <div>
            <span className="mb-2 block text-[13px] font-semibold text-[#51627f]">Type</span>
            <div className="relative flex h-[46px] items-center rounded-[12px] border border-[#d7deee] bg-[#f8faff] px-3">
              <select
                value={q.type}
                onChange={(e) =>
                  onChange({ ...q, type: e.target.value as QuizQuestionType })
                }
          ...
          {/* Marks */}
          <div>
            <span className="mb-2 block text-[13px] font-semibold text-[#51627f]">Marks</span>
            <input
              type="number"
              min={1}
              value={q.marks}
              onChange={(e) => onChange({ ...q, marks: Number(e.target.value) || 1 })}
              className="h-[46px] w-full rounded-[12px] border border-[#d7deee] bg-[#f8faff] px-4 text-[14px] text-[#264267] outline-none"
            />
          </div>

          {/* Correct option (multiple_choice only) */}
            ...
            {q.options.map((opt, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="h-[40px] flex-1 rounded-[10px] border border-[#d7deee] bg-[#f8faff] px-3 text-[14px] text-[#264267] outline-none placeholder:text-[#8d99b1]"
                />
              </div>
            ))}
          </div>
...
// ---------------------------------------------------------------------------
// QuizSettingsModal — create or edit a quiz with inline questions
// ---------------------------------------------------------------------------
function QuizSettingsModal({
  ...
  // Questions — pre-populate from existing quiz
  ...
  function updateQuestion(key: string, updated: DraftQuestion) {
    setQuestions((prev) => prev.map((q) => (q.key === key ? updated : q)));
  }

  function removeQuestion(key: string) {
    setQuestions((prev) =>
      prev.filter((q) => q.key !== key).map((q, i) => ({ ...q, order: i + 1 }))
    );
  }
  ...
  async function handleSave() {
    ...
    const questionPayload: QuizQuestionInput[] = questions.map((q) =>
      q.type === "multiple_choice"
        ? {
  ...
  return (
    <CourseModal closeHref="#" maxWidthClassName="max-w-[900px]">
      ...
      <div className="p-7 pr-14 sm:p-9 sm:pr-16">
        ...
        <p className="mt-2 text-[16px] text-[#4b6182]">
          {isEditing ? "Update" : "Configure"} quiz settings and questions for{" "}
        ...
        <div className="mt-10 space-y-6">
          ...
          <section className="rounded-[18px] bg-[#f6f8ff] p-6">
            <div className="flex items-center justify-between gap-4">
              <h3 className="text-[16px] font-extrabold tracking-[-0.03em] text-[#1a2f51]">
                Questions
                <span className="ml-2 rounded-full bg-[#e6f7ee] px-2 py-0.5 text-[12px] font-bold text-[#0f8751]">
                  {questions.length}
                </span>
              </h3>
              <button
                type="button"
                onClick={addQuestion}
                className="flex items-center gap-2 rounded-[10px] border border-[#bfe6d2] bg-white px-4 py-2 text-[13px] font-semibold text-[#0f8751] hover:bg-[#f1fdf6]"
              >
    ...
    <div className="space-y-6">
      <aside className="rounded-[22px] border border-[#dfe6f7] bg-white p-6 shadow-[0_18px_35px_rgba(180,193,229,0.08)]">
        ...
        <dl className="mt-7 space-y-4 text-[16px] text-[#576a88]">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <dd className="font-semibold text-[#22314c]">{totalQuestions}</dd>
          </div>
          {withoutQuiz > 0 && (
      ...
      <aside className="rounded-[22px] border border-[#dfe6f7] bg-[#eef1fb] p-6">
        ...
        <ul className="mt-5 space-y-3 text-[14px] leading-6 text-[#576a88]">
          <li>• One quiz per module — add multiple choice or theory questions.</li>
          <li>• Questions are saved together with the quiz in one request.</li>
          <li>
            • Quizzes marked <strong>Draft</strong> are hidden from students.
          </li>
          <li>• Use the edit (✏) icon to update quiz settings or questions.</li>
        </ul>
      </aside>
    </div>
  );
      ...
      {!collapsed && (
        <div className="border-t border-[#e8edf7] bg-white p-4 sm:p-7">
          ...
          {quiz.questions.length > 0 && (
            <div className="mt-6">
              <p className="text-[13px] font-extrabold uppercase tracking-[0.08em] text-[#7e8da6]">
              ...
              <div className="mt-3 space-y-2">
                {quiz.questions.map((q, qi) => (
                  <div
                    key={q.id}
                    className="flex items-start gap-3 rounded-[12px] border border-[#e8edf7] bg-[#f8faff] px-4 py-3"
                  >
                    <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#eef1fb] text-[11px] font-extrabold text-[#667792]">
                      {qi + 1}
                    </span>
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 text-[12px] text-[#7e8da6]">
                        {q.type === "multiple_choice" ? "Multiple Choice" : "Theory"} · {q.marks} mark{q.marks !== 1 ? "s" : ""}
                      </p>
                    </div>
                  </div>
                ))}
    ...
    <article className="overflow-hidden rounded-[22px] border border-dashed border-[#dceadf] bg-white">
      ...
      <div className="px-6 py-10 text-center">
        ...
        <p className="mx-auto mt-2 max-w-[380px] text-[14px] leading-6 text-[#71819d]">
          Add a quiz with questions to test students on this module.
        </p>
        <button
          type="button"
...
export function QuizBuilderView({
   ...
   *
   * Create: POST /courses/modules/:moduleId/quiz  with full questions[]
   * Update: PUT  /courses/modules/:moduleId/quiz  with full questions[]
   *
   * Both payloads are identical in shape — the questions array replaces
   * whatever was there before on update.
   */
async function handleSaveQuiz(
  ...
  setQuizError(null);
  try {
    const savedQuiz = existingQuizId
      ? await updateQuiz(moduleId, payload, session?.token)
      : await createQuiz(moduleId, payload, session?.token);

    // Refresh modules from server first
    await onRefresh();

    // fetchModules returns quiz metadata WITHOUT questions array.
    // Patch the questions from the API save response back in after refresh.
    if (savedQuiz) {
      setModules((prev) =>
        prev.map((m) => {
    ...
    setQuizError(err instanceof Error ? err.message : "Unable to save quiz.");
    throw err; // re-throw so QuizSettingsModal catches it and shows error
  } finally {
...
// ---------------------------------------------------------------------------
// CourseContentUploadContent — main page
// ---------------------------------------------------------------------------
function CourseContentUploadContent() {
  ...
  async function handleSaveLesson(payload: {
    ...
    const instructorId = (session as any)?.user?.id;
...
<QuizBuilderView
  ...
  setModules={setModules}   // ADD THIS
/>
        )}
      </div>
